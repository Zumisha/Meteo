...
function getData() {
    ...
    request.open("GET", "http://192.168.1.228/getDebugData", true);
...
function UpdateChart(){
    ...
    let Atm = [];
    let Carb = [];
    let step = Math.ceil(Data.Dates.length/(numPoints+1));
    let dateIndex = Data.Dates.length - numPoints*step - 1;
    let j = 0;
    for (let i=0; i<numPoints; ++i)
    {
        ...
        Atm.push(0);
        Carb.push(0);
        for (; j< Math.(dateIndex + step/2); ++j)
        {
            Temp[i]+=Data.Temperature[j];
            Hum[i]+=Data.Humidity[j];
            Atm[i]+=Data.AtmPressure[j];
            Carb[i]+=Data.CarbonDioxide[j];
        }
        Dates.push(Data.Dates[j-step/2]);
        Temp[i] /= step;
        Hum[i] /= step;
        Atm[i] /= step;
        Carb[i] /= step;
    }
    Dates.push(Data.Dates[-1]);*/
